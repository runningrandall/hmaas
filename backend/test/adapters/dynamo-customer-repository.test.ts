...
describe('DynamoCustomerRepository', () => {
    ...
    describe('list', () => {
        it('should return paginated list of customers', async () => {
            mockEntity.query.byOrgCustomers.mockReturnValue({
        ...
        it('should pass limit and cursor options', async () => {
            ...
            mockEntity.query.byOrgCustomers.mockReturnValue({ go: mockGo });

            const result = await repo.list('org-test-123', { limit: 5, cursor: 'some-cursor' });

            expect(mockEntity.query.byOrgCustomers).toHaveBeenCalledWith({ organizationId: 'org-test-123' });
