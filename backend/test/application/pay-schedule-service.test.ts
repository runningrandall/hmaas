...
describe('PayScheduleService', () => {
    ...
    describe('createPaySchedule', () => {
        it('should create pay schedule and record metric without publishing an event', async () => {
            ...
            const result = await service.createPaySchedule(request as any);
    ...
    describe('getPaySchedule', () => {
        it('should return pay schedule when found', async () => {
            const schedule = { payScheduleId: 'psched-1', name: 'Bi-Weekly', frequency: 'bi-weekly' };
            mockRepo.get.mockResolvedValue(schedule);

            const result = await service.getPaySchedule('psched-1');

            expect(result).toEqual(schedule);
            expect(mockRepo.get).toHaveBeenCalledWith('psched-1');
        });

        it('should throw AppError 404 when pay schedule not found', async () => {
            mockRepo.get.mockResolvedValue(null);

            await expect(service.getPaySchedule('missing')).rejects.toThrow(AppError);
            await expect(service.getPaySchedule('missing')).rejects.toMatchObject({ statusCode: 404 });
        });
    });

    describe('listPaySchedules', () => {
        it('should delegate to repo.list', async () => {
            const paginated = { items: [{ payScheduleId: 'psched-1' }], count: 1 };
            mockRepo.list.mockResolvedValue(paginated);

            const result = await service.listPaySchedules({ limit: 10 });
    ...
    describe('updatePaySchedule', () => {
        it('should update pay schedule and return updated without publishing event', async () => {
            const existing = { payScheduleId: 'psched-1', name: 'Bi-Weekly', frequency: 'bi-weekly' };
            const updated = { payScheduleId: 'psched-1', name: 'Bi-Weekly Updated', frequency: 'bi-weekly' };
            mockRepo.get.mockResolvedValue(existing);
            mockRepo.update.mockResolvedValue(updated);

            const result = await service.updatePaySchedule('psched-1', { name: 'Bi-Weekly Updated' } as any);
        ...
        it('should throw 404 if pay schedule not found', async () => {
            ...
            await expect(service.updatePaySchedule('missing', { name: 'x' } as any)).rejects.toMatchObject({ statusCode: 404 });
        });
    });
    ...
    describe('deletePaySchedule', () => {
        it('should delete pay schedule without publishing event', async () => {
            ...
            await service.deletePaySchedule('psched-1');

            expect(mockRepo.delete).toHaveBeenCalledWith('psched-1');
        });
    });
});
