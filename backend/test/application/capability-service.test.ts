...
describe('CapabilityService', () => {
    ...
    describe('createCapability', () => {
        it('should create capability, publish CapabilityCreated event, and record metric', async () => {
            ...
            const result = await service.createCapability(request as any);

            expect(result).toEqual(created);
            expect(mockRepo.create).toHaveBeenCalledOnce();
            expect(mockPublisher.publish).toHaveBeenCalledWith('CapabilityCreated', {
    ...
    describe('getCapability', () => {
        it('should return capability when found', async () => {
            const capability = { capabilityId: 'cap-1', employeeId: 'emp-1', level: 'expert' };
            mockRepo.get.mockResolvedValue(capability);

            const result = await service.getCapability('cap-1');

            expect(result).toEqual(capability);
            expect(mockRepo.get).toHaveBeenCalledWith('cap-1');
        });

        it('should throw AppError 404 when capability not found', async () => {
            mockRepo.get.mockResolvedValue(null);

            await expect(service.getCapability('missing')).rejects.toThrow(AppError);
            await expect(service.getCapability('missing')).rejects.toMatchObject({ statusCode: 404 });
        });
    });

    describe('listCapabilitiesByEmployee', () => {
        it('should delegate to repo.listByEmployeeId', async () => {
            const paginated = { items: [{ capabilityId: 'cap-1', employeeId: 'emp-1' }], count: 1 };
            mockRepo.listByEmployeeId.mockResolvedValue(paginated);

            const result = await service.listCapabilitiesByEmployee('emp-1', { limit: 10 });
    ...
    describe('deleteCapability', () => {
        it('should fetch capability first then delete it', async () => {
            const capability = { capabilityId: 'cap-1', employeeId: 'emp-1' };
            mockRepo.get.mockResolvedValue(capability);
            mockRepo.delete.mockResolvedValue(undefined);

            await service.deleteCapability('cap-1');

            expect(mockRepo.get).toHaveBeenCalledWith('cap-1');
            expect(mockRepo.delete).toHaveBeenCalledWith('cap-1');
        });

        it('should throw 404 if capability not found before deleting', async () => {
            mockRepo.get.mockResolvedValue(null);

            await expect(service.deleteCapability('missing')).rejects.toMatchObject({ statusCode: 404 });
