...
export class PaymentMethodService {
    ...
    async getPaymentMethod(paymentMethodId: string): Promise<PaymentMethod> {
        const paymentMethod = await this.repository.get(paymentMethodId);
        if (!paymentMethod) {
    ...
    async listPaymentMethodsByCustomer(customerId: string, options?: PaginationOptions): Promise<PaginatedResult<PaymentMethod>> {
    ...
    async deletePaymentMethod(paymentMethodId: string): Promise<void> {
        ...
        await this.repository.delete(paymentMethodId);
