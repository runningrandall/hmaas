...
import { SetOrgSecretSchema } from "../../lib/organization-schemas";
...
const baseHandler = async (event: APIGatewayProxyEvent, context: any): Promise<APIGatewayProxyResult> => {
    ...
    if (!key) {
        throw new AppError("Missing secret key", 400);
    }
    const body = event.body as unknown as any;
    if (!body) {
        throw new AppError("Missing request body", 400);
    }
    const parseResult = SetOrgSecretSchema.safeParse(body);
    if (!parseResult.success) {
        logger.warn("Validation failed", { issues: parseResult.error.issues });
        metrics.addMetric('ValidationErrors', MetricUnit.Count, 1);
        throw parseResult.error;
    }
    await service.setSecret(organizationId, key, parseResult.data.value);
    return { statusCode: 200, body: JSON.stringify({ message: "Secret set successfully" }) };
};
