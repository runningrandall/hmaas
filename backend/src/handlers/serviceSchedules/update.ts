...
import { UpdateServiceScheduleSchema } from "../../lib/service-schedule-schemas";
...
const baseHandler = async (event: APIGatewayProxyEvent, context: any): Promise<APIGatewayProxyResult> => {
    ...
    const parseResult = UpdateServiceScheduleSchema.safeParse(body);
    if (!parseResult.success) {
        logger.warn("Validation failed", { issues: parseResult.error.issues });
        metrics.addMetric('ValidationErrors', MetricUnit.Count, 1);
        throw parseResult.error;
    }
    const result = await service.updateServiceSchedule(organizationId, serviceScheduleId, parseResult.data);
